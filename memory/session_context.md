...
## 最終更新: 2026-07-16（SEC-001/003・SEC-002 完了 + パイプライン再設計 + 本番デプロイ一括 完了）

### 🧭 次セッション冒頭でまず読む「現在地」
- **家裁プロダクトは本番デプロイまで完了・区切り済み**。コード/DB/env/Stripe本番/Spend Limit すべて反映済み（下記「本番デプロイ一括=完了」参照）。
- **唯一の未消化（ダイチ手動）**: Stripe 本番モードで実カード購入を1回通し、checkout→webhook→クレジット付与を確認（実決済のためリード不可）。この検証中/後の不具合対応が来る可能性あり。
- **backlog 未対応の残り**: SEC-004（モデルID集約・低）／ MON-002（広告・低）／ MON-001 サブスク月額（将来）。急ぎは無い。
- **パイプラインは新方式**（リード編成 × harness Agent サブエージェント）。`agents.sh` は deprecated。詳細 [[feedback-pipeline-runner]]。方針を聞くときは AskUserQuestion でなく普通のテキストで（bypass だとカードが出ない）。

### 現在のブランチ・PR 状態
- 現ブランチ: `main`（`9305101`・ローカル同期済み・作業ツリークリーン・**オープン PR なし**）
- マージ済み最新: PR #64（パイプライン再設計）／ PR #65（SEC-002）／ PR #66（lib/stripe.ts 遅延初期化）
