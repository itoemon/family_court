...
dev サーバーは **`scripts/agents.sh` が node20 環境で既に起動済み**です。本リポジトリは Next.js 16（node ≥ 20.9.0 必須）であり、パイプライン各エージェントは node18 配下で動くため、サーバーの起動・停止は agents.sh が担当します。**あなたは `npm run dev` を起動・停止しないこと。**

応答することだけ確認してテストに進んでください:
```bash
...
```
30 秒待っても応答しない場合はサーバー起動に失敗しています。その旨をレポートに「実施不可」として記録し、テストを中断してください（自分で `npm run dev` を起動し直さないこと）。
...
dev サーバーの停止は `scripts/agents.sh` が行います。`kill` 等は実行しないこと。
