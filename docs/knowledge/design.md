...
実装状況: `lib/judge.ts` は対応済み。`lib/defense.ts` は `escapeXml` 適用済みだが、`dialogHistory` の各 `content` に `truncate` が未適用（D-1 参照）。

### D-1. `lib/defense.ts` — `dialogHistory.content` の `truncate` 未適用（未対応）

`lib/defense.ts` の `generateDefenseResponse`（47 行目）および `generateDraft`（81 行目）で `dialogHistory` を展開する際、`escapeXml` は適用済みだが `truncate` が未適用である。過去の発言内容が無制限長のままプロンプトに展開されるため、長大な入力によるプロンプトインジェクションの攻撃面が残存する。

対応方針: `dialogHistory.map` の `a.content` を `escapeXml(truncate(a.content, 500))` に変更する。`truncate` 関数は `lib/judge.ts` または同ファイルに既存のものを使い回す（インポート追加か同ファイルへのコピーで対応）。

### D-2. `defense/route.ts` — 認証ユーザーパスの try-catch 漏れ（未対応）

`app/api/cases/[id]/defense/route.ts` の `resolveAuth` 関数内、認証ユーザーパス（L15–L25: `createSessionClient` 呼び出し〜ユーザー ID 判定）が try-catch の外にある。Supabase クライアント初期化失敗時に未捕捉例外が発生しうる。ゲストパス（L27–L37）はすでに try-catch 済み。

対応方針: 認証ユーザーパス（`createSessionClient` から `return { user, userId, ... }` まで）を try-catch で囲み、例外時は `{ error: "サーバー設定エラーが発生しました。管理者に連絡してください。", status: 500 }` を返す。`argument/route.ts` の try-catch パターン（L26–L48）を踏襲すること。
...
### D-5. `judge_messages` への空文字列挿入（未対応）

`app/api/cases/[id]/route.ts`（L96・L130）および `app/api/cases/[id]/argument/route.ts`（L143）で `generateJudgeMessage` の戻り値を空チェックせず INSERT している。戻り値が空文字列の場合、本文なしのバブルが DB に挿入されクライアントで表示される。

対応方針: 各 INSERT の直前に `if (!content) return;` または `if (!content.trim()) { /* skip */ continue; }` を追加する。既存の `argument/route.ts` における try-catch ブロック内で `content` を使用している L143 が対象。`route.ts` では L96 と L130 の両 INSERT 箇所が対象。
...
|--------|---------|---------|-------|------|
| D-1 | `lib/defense.ts` | 修正（truncate 追加 × 2 箇所） | 最小 | **未対応** |
| D-2 | `app/api/cases/[id]/defense/route.ts` | 修正（try-catch 拡張） | 小 | **未対応** |
| D-3 | `app/api/clear-flash/route.ts` | — | — | 対応済み |
| D-4 | `tests/e2e/security-fixes.spec.ts` | — | — | 対応済み |
| D-5 | `app/api/cases/[id]/route.ts`・`argument/route.ts` | 修正（空チェック追加 × 3 箇所） | 最小 | **未対応** |
